...
int main(int argc, char *argv[]) {  
  ...
  uint32_t num_points = count * num_tasks; //each task data
  uint32_t buf_size   = num_points * sizeof(int32_t); 

  std::cout << "num_cores="<<num_cores<<" num_warps="<<num_warps << " num_threads="<<num_threads<<" num_tasks="<<num_tasks<<" count="<<count;
